...
export async function POST(request: Request) {
  try {
    ...
    const wpRes = await fetch(`https://lexhoy.com/wp-json/wp/v2/despacho?object_id=${objectId}`,
      {
        headers: {
    ...
    if (!wpData || !wpData.length) {
      return NextResponse.json({ error: 'Despacho no encontrado en WordPress' }, { status: 404 });
    }
    ...
    const despachoSupabase = {
      ...
      email_contacto: despachoWP.meta?.email_contacto || '',
      // Agrega aquí más campos según el esquema de Supabase
    };
    ...
    if (result.error) {
      return NextResponse.json({ error: 'Error guardando en Supabase', details: String(result.error) }, { status: 500 });
    }
