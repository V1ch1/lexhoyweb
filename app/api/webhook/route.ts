...
export async function POST(request: Request) {
  try {
    ...
    if (expectedSecret && authHeader !== `Bearer ${expectedSecret}`) {
      console.error(
        "❌ No autorizado - Secreto de webhook inválido o faltante"
      );
...
async function handleDespachoUpdate(despachoData: DespachoData) {
  try {
    ...
    const estadoVerificacionMeta =
      despachoData.meta?._despacho_estado_verificacion;
    ...
    if (
      estadoVerificacionMeta === "verificado" ||
      isVerifiedMeta === "1" ||
      String(isVerifiedMeta) === "1"
    ) {
    ...
    const updateData = {
      ...
      wordpress_id:
        typeof despachoData.id === "number"
          ? despachoData.id
          : parseInt(String(despachoData.id)),
