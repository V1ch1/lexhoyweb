...
export async function GET(request: Request) {
  ...
  // Excepción: si query es "*", buscar todos los despachos
  if (!query && !id) {
  ...
  // Si query es "*", buscar con término genérico para obtener muchos resultados
  ...
  try {
    ...
    // Búsqueda por texto
    // Usar searchQuery que ya fue procesado anteriormente
...
async function buscarDespachoPorId(id: string) {
  ...
  if (!response.ok) {
    ...
    console.error(
      "❌ [WordPress] Error en la búsqueda:",
      response.status,
      errorText
    );
    return null;
...
async function buscarDespachosPorTexto(query: string): Promise<DespachoWP[]> {
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        `❌ [WordPress] Error en la respuesta: ${response.status}`,
        errorText
      );
      throw new Error(
    ...
    if (error instanceof Error) {
      if (error.name === "AbortError") {
        throw new Error(
          "La búsqueda tardó demasiado tiempo. Por favor, inténtalo de nuevo."
        );
      }

      if ("code" in error && error.code === "UND_ERR_SOCKET") {
        throw new Error(
          "Error de conexión con el servidor de WordPress. Por favor, verifica tu conexión a internet."
        );
      }
