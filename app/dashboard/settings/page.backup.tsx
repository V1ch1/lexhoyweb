...
export default function SettingsPage() {
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      case 'profile':
        return <ProfileTab profile={profile} onUpdate={setProfile} />;
      case 'password':
        return <PasswordTab />;
      case 'notifications':
        return <NotificationsTab />;
      case 'mis-despachos':
        return <MisDespachosTab />;
      case 'privacy':
        return <PrivacyTab />;
      case 'sessions':
        return <SessionsTab />;
...
const SettingsPage = () => {
  ...
  const user = authUser ? safeUser(authUser) : null;
  const [loading, setLoading] = useState(false);
  const [showCurrentPassword, setShowCurrentPassword] = useState(false);
  const [showNewPassword, setShowNewPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  const [message, setMessage] = useState<{type: string; text: string} | null>(null);
  const [userDespachos, setUserDespachos] = useState<Array<{id: string; [key: string]: any}>>([]);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [despachoToDelete, setDespachoToDelete] = useState<{ id: string; nombre: string } | null>(null);

  // Tabs configuration
  const tabs: Tab[] = [
    { id: 'profile' as SettingsTab, name: 'Perfil', icon: UserIcon, visible: true },
    { id: 'password' as SettingsTab, name: 'Contraseña', icon: KeyIcon, visible: true },
    { id: 'notifications' as SettingsTab, name: 'Notificaciones', icon: BellIcon, visible: true },
    { id: 'mis-despachos' as SettingsTab, name: 'Mis Despachos', icon: BuildingOfficeIcon, visible: true },
    { id: 'privacy' as SettingsTab, name: 'Privacidad', icon: ShieldCheckIcon, visible: true },
    { id: 'sessions' as SettingsTab, name: 'Sesiones', icon: ComputerDesktopIcon, visible: true },
  ];
  ...
  const handleProfileUpdate = async (data: Partial<UserProfile>) => {
    ...
    try {
      ...
      const updatedUser = await userService.updateUser(user.id, {
        nombre: data.nombre || profileData.nombre,
        apellidos: data.apellidos || profileData.apellidos,
        email: data.email || profileData.email,
        telefono: data.telefono || profileData.telefono,
      });
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: error.message || "Error al cambiar la contraseña. Por favor, inténtalo de nuevo.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleNotificationsSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setMessage({
        type: "error",
        text: "Usuario no autenticado. Por favor, inicia sesión de nuevo.",
      });
      return;
    }
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: "Error al actualizar las notificaciones. Por favor, inténtalo de nuevo.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteDespacho = async (despachoId: string) => {
    ...
    try {
      setLoading(true);
      // Call API to remove the user from the law firm
      // This is a placeholder - implement the actual API call
      // await userService.removeFromDespacho(user.id, despachoId);
      ...
      setMessage({
        type: "success",
        text: "Has sido eliminado del despacho correctamente.",
      });
    } catch (error) {
      console.error("Error removing from despacho:", error);
        ...
        return (
          <div className="space-y-6">
            ...
            <p className="text-gray-600">
              Aquí podrás configurar las opciones de privacidad de tu cuenta.
            </p>
      ...
      case 'sessions':
        return (
          <div className="space-y-6">
            ...
            {/* Add active sessions components here */}
          </div>
        );
      case 'privacy':
        return <div className="p-4">Configuración de privacidad</div>;
      case 'sessions':
        return <div className="p-4">Gestión de sesiones</div>;
      default:
        return null;
    }
  };
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-lg shadow">
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8 px-6 overflow-x-auto" aria-label="Tabs">
              {tabs
                .filter(tab => tab.visible(userDespachos))
                .map((tab) => {
                  const Icon = tab.icon;
