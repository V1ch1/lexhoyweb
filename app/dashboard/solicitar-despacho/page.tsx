...
export default function SolicitarDespacho() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setSuccess(null);
    try {
      const res = await fetch(`/api/search-despachos?query=${encodeURIComponent(nombre)}`);
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="mb-8">
        <h3 className="text-lg font-semibold mb-2">Mis solicitudes de despacho</h3>
            ...
            <tbody>
              {solicitudesPendientes.map(solicitud => (
                <tr key={solicitud.id} className="border-b">
                  <td className="px-4 py-2">{decodeHtml(solicitud.despacho_nombre || solicitud.id)}</td>
                  <td className="px-4 py-2">{decodeHtml(solicitud.despacho_localidad || '-')}</td>
                  <td className="px-4 py-2">{decodeHtml(solicitud.despacho_provincia || '-')}</td>
                  ...
                  <td className="px-4 py-2 flex gap-2 items-center">
                    <span className="bg-gray-100 text-gray-800 px-2 py-1 rounded">{solicitud.estado || 'Sin estado'}</span>
                  </td>
                  <td className="px-4 py-2">
      ...
      {(!loading && results.length === 0 && nombre) && (
        <p className="text-gray-500 mt-4">No se encontraron despachos con ese nombre.</p>
      )}
    </div>
  );
}
